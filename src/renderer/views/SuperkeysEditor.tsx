...
import {
	SuperKeysFeatures,
	SuperkeyActions,
} from "@Renderer/modules/Superkeys";
import { KeyPickerKeyboard } from "@Renderer/modules/KeyPickerKeyboard";

// Types
import {
	SuperkeysEditorInitialStateType,
	SuperkeysEditorProps,
} from "@Renderer/types/superkeyseditor";
...
function SuperkeysEditor(props: SuperkeysEditorProps) {
	...
	const initialState: SuperkeysEditorInitialStateType = {
		...
		futureSK: [],
		futureSSK: 0,
		currentLanguageLayout: getLanguage(
			store.get("settings.language") as string,
		),
	...
	const macroTranslator = (raw: string) => {
		...
		return macros.map((macro, i) => {
			...
			return {
				...
				name: stored[i]?.name,
				macro: macro.actions
					.map((k) => keymapDB.parse(k.keyCode as number).label)
					.join(" "),
			};
		});
	};
	...
	const superTranslator = (raw: string) => {
		...
		if (superArray.length < 1) {
			log.info(
				"Discarded Superkeys due to short length of string",
				raw,
				raw.length,
			);
			return [
				{
					actions: [53, 2101, 1077, 41, 0],
					name: "Welcome to superkeys",
					id: superindex,
				},
			];
		}
		...
		try {
			...
			try {
				kbtype =
					...
					currentDevice.device.info.keyboardType === "ISO"
						? "iso"
						: "ansi";
			} catch (error) {
				return false;
			...
			const onlyCustom = Boolean(
				parseInt(await currentDevice.command("keymap.onlyCustom"), 10),
			);
			...
			const layerSize =
				currentDevice.device.keyboard.rows *
				currentDevice.device.keyboard.columns;
	...
	const superkeyMap = (superkeys: SuperkeysType[]) => {
		if (
			...
			(superkeys.length === 1 &&
				superkeys[0].actions.length === 1 &&
				superkeys[0].actions[0] === 0)
		) {
		...
		keyMap = keyMap.map((sky: SuperkeysType) => {
			...
			if (sk.actions.length < 5)
				sk.actions = sk.actions.concat(Array(5 - sk.actions.length).fill(1));
			return sk;
		});
		log.info("Third", JSON.parse(JSON.stringify(keyMap)));
		const mapped = keyMap
			.map((superkey: SuperkeysType) =>
				superkey.actions.filter((act) => act !== 0).concat([0]),
			)
			.flat()
			.concat([0])
			.join(" ")
	...
	const writeSuper = async () => {
		...
		try {
			...
			const backup = await bkp.DoBackup(
				commands,
				neurons[nIdx].id,
				currentDevice,
			);
	...
	const SortSK = (newSuper: SuperkeysType[], newID: number) => {
			...
			listToDecrease.push(
				keymap.custom
					.map((l, c) =>
						l
							.map((k, i) => {
								if (k.keyCode === key.id + 53980)
									return { layer: c, pos: i, sk: key.id + 53980 };
								return undefined;
							})
		...
		for (let i = 0; i < listToDecrease.length; i += 1) {
			keymap.custom[listToDecrease[i].layer][listToDecrease[i].pos] =
				keymapDB.parse(listToDecrease[i].sk - 1);
		}
		state.keymap = keymap;
		state.superkeys = newSuper;
		state.selectedSuper = newID;
	...
	const checkKBSuperkeys = (
		newSuper: SuperkeysType[],
		newID: number,
		SKC: number,
	) => {
		const { keymap, selectedSuper, superkeys } = state;
		...
		const LOK = keymap.custom
			.map((l, c) =>
				l
					.map((k, i) => {
						if (k.keyCode === SKC)
							return { layer: c, pos: i, superIdx: SKC, newKey: 0 };
						return undefined;
					})
					.filter((x) => x !== undefined),
			)
	...
	const RemoveDeletedSK = () => {
				...
				keymap.custom
					.map((l, c) =>
						l
							.map((k, i) => {
								if (k.keyCode === key.id + 53980)
									return { layer: c, pos: i, superIdx: key.id + 53980 };
								return undefined;
							})
							.filter((x) => x !== undefined),
					)
		...
		for (let i = 0; i < listToDelete.length; i += 1) {
			keymap.custom[listToDelete[i].layer][listToDelete[i].pos] =
				keymapDB.parse(0);
		}
		listToDecrease = listToDecrease.flat();
		...
		for (let i = 0; i < listToDecrease.length; i += 1) {
			keymap.custom[listToDecrease[i].layer][listToDecrease[i].pos] =
				keymapDB.parse(listToDecrease[i].superIdx - 1);
		}
		state.keymap = keymap;
	...
	const updateScroll = useCallback((e: WheelEvent) => {
		...
		if (!(e.target as HTMLElement).outerHTML.includes('<div role="option"'))
	...
	const listOfSKK = listToDelete.map(({ layer, pos, superIdx }) => (
		<li
			key={`${layer}-${pos}-${superIdx}`}
			className="titles alignvert"
		>{`Key in layer ${layer + 1} and pos ${pos}`}</li>
	));
				...
				<DialogContent>
					...
					<div className="px-6 pb-2 mt-2">
						{listOfSKK && (
							<>
								...
								<div className="mt-1 mb-4">
									<ul className="mb-[4px] text-ssm list-disc pl-[24px] text-gray-500 dark:text-gray-100">
										{listOfSKK}
									</ul>
								</div>
							</>
						...
						<p className="text-sm text-gray-500 dark:text-gray-25">
							By pressing <strong>Remove</strong> you will replace those
							superkeys with <strong>NO KEY</strong> on the keyboard layout.
						</p>
					</div>
					<DialogFooter>
